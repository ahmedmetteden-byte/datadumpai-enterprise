    ...
    def _generate_markdown(
        ...
        *,
        ...
        prompt = (
            ...
            "stop at restating the fact. Then a line `**Basis:** <value>` where <value> is EXACTLY "
            "one of these three literal phrases — not a paraphrase, not a different label such as "
            "'Verified Calculations' — `Source fact` (stated directly by a source document), "
            "`Calculated result` (computed in the Verified Calculations block above), or "
            "`Analytical inference` (your own interpretation, not stated or computed anywhere in "
            "the evidence) — never present an inference as if it were a stated fact. Then "
            "`**Confidence:** High/Medium/Low "
            "— <one-clause reason>` reflecting how well-supported the finding is by the evidence (a "
            "Calculated-result finding grounded in the Verified Calculations block is High by "
            "default; an inference resting on a single ambiguous mention is Low), then `**Source:** "
            "<filename(s), plural if more than one document supports it>`.\n\n"
            "## Detailed Analysis\n"
            ...
            "likely impact. If the evidence surfaces no material risks, state that plainly rather "
            "than manufacturing a generic one.\n\n"
            "## Opportunities\n"
